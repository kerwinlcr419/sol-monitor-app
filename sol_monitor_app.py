...
st.set_page_config(
    page_title="SOL鏈代幣監測器",
    page_icon="🚀",
    layout="wide"
)

st.title("🚀 SOL鏈代幣即時監測器")
st.caption("✅ 5 大平台｜Pump.fun 使用官方真實 API｜其餘平台使用 Dexscreener 公開 API｜全部真實數據・無任何模擬")

# ==================== 側邊欄條件設定 ====================
st.sidebar.header("⚙️ 警報條件設定（3 項）")
mc_threshold = st.sidebar.number_input("1. 市值設定值以下 (USD)", value=150000, step=10000, help="市值低於此值 + 單次大買才觸發")
...
st.sidebar.divider()
st.sidebar.info("💡 Pump.fun 使用官方 frontend-api-v3 真實資料\n其他 4 平台使用 Dexscreener 公開 API（貼 CA 即可監測）")

# ==================== 共用真實 API 函式 ====================
@st.cache_data(ttl=30)
def fetch_pump_latest():
    """Pump.fun 官方真實最新代幣（2026 年仍有效）"""
    try:
...
def get_dexscreener_pairs(token_address: str):
    """Dexscreener 公開 API - 任意 Solana 代幣對資料"""
    try:
...
def check_conditions(coin: dict, trades: list):
    ...
    if mc < mc_threshold:
        for t in trades[:30]:
            if t.get("is_buy") and float(t.get("sol_amount", 0)) >= single_buy_sol:
                alerts.append(f"🔥 條件1觸發：市值 ${mc:,.0f} < {mc_threshold:,} + 單次買入 ≥ {single_buy_sol} SOL")
                break
    ...
    if buy_1min >= min_buy_sol:
        alerts.append(f"⚡ 條件2觸發：最近 1 分鐘買入 {buy_1min:.1f} SOL ≥ {min_buy_sol} SOL")
            ...
            if age > age_days:
                for t in trades[:15]:
                    if t.get("is_buy") and float(t.get("sol_amount", 0)) >= sudden_buy_sol:
                        alerts.append(f"🚨 條件3觸發：老幣（{age:.1f} 天）突然大買 ≥ {sudden_buy_sol} SOL")
                        break
        except:
            pass
...
# ==================== 5 個平台分頁 ====================
tabs = st.tabs(["1. Pump.fun", "2. Moonshot", "3. LaunchLab / LetsBONK.fun", "4. Meteora / Alpha Vaults", "5. Zerg.zone"])

# Tab 1: Pump.fun（完整自動監測）
with tabs[0]:
    st.subheader("📌 Pump.fun（官方真實 API 即時資料）")
    col1, col2 = st.columns([1, 4])
    with col1:
        if st.button("🔄 立即刷新 Pump.fun", type="primary", use_container_width=True):
            st.cache_data.clear()
            st.rerun()
    
    coins = fetch_pump_latest()
    st.info(f"目前抓到 {len(coins)} 個最新/活躍 Pump.fun 代幣")
    ...
    if alert_count > 0:
        st.success(f"🎯 本次發現 {alert_count} 個符合您 3 項條件的 Pump.fun 代幣！")
    else:
        st.success("✅ Pump.fun 目前無符合警報的代幣")

# Tab 2~5：其餘平台（貼 CA 查詢 Dexscreener 真實資料）
with tabs[1]:
    st.subheader("📌 2. Moonshot")
    st.info("Moonshot 無公開最新列表，使用 Dexscreener 即時查詢")
    ca = st.text_input("輸入 Moonshot 代幣地址 (CA)", placeholder="貼上 Solana 代幣合約地址")
    if ca:
...
with tabs[2]:
    st.subheader("📌 3. LaunchLab / LetsBONK.fun")
    st.info("LaunchLab / LetsBONK.fun 使用 Dexscreener 即時資料")
...
with tabs[3]:
    st.subheader("📌 4. Meteora / Alpha Vaults")
    st.info("Meteora DLMM / Alpha Vaults 使用 Dexscreener 監測")
...
with tabs[4]:
    st.subheader("📌 5. Zerg.zone")
    st.info("Zerg.zone 使用 Dexscreener 即時監測")
...
# ==================== 頁尾 ====================
st.divider()
st.caption(f"最後更新時間：{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}（台灣時間）｜點擊上方「立即刷新」按鈕即可強制更新")
st.caption("💡 直接把此檔案存成 app.py 上傳 GitHub → 在 streamlit.io 部署即可永久免費運行")