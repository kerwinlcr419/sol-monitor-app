...
# ====================== 主畫面條件設定（可直接手動調整） ======================
st.subheader("⚙️ 條件設定（點擊展開後可手動修改數值）")
with st.expander("🔧 點我展開設定 3 項條件", expanded=True):  # 預設展開
...
with col1:
    st.metric("條件1", f"${mcap_threshold:,} USD + {buy_amount1} SOL")
with col2:
    st.metric("條件2", f"{minute_buy_threshold} SOL / 分鐘")
with col3:
    st.metric("條件3", f"{age_threshold_days} 天以上 + {sudden_buy_amount} SOL")
...
def copy_ca_button(ca):
    st.markdown(
        ...
        this.innerHTML='✅ 已複製！'; 
        setTimeout(() => {{this.innerHTML='📋 複製 CA'; this.style.backgroundColor='#00cc66';}}, 2000); 
        return false;"
        style="background-color:#00cc66; color:white; padding:10px 20px; border-radius:8px; 
        text-decoration:none; display:inline-block; font-size:16px; font-weight:bold; margin:5px 0;">
        📋 複製 CA
        </a>
        """,
...
st.success("✅ 現在可以直接在主畫面手動調整所有條件數值了！")
st.caption("展開「🔧 點我展開設定」即可修改")